...
const Screen1 = () => {
  ...
  const onPressChanel = async (chanel) => {
    try {
      const state = channelsData[chanel].status === 0 ? 1 : 0;
      setChannelsData({
        ...channelsData,
        [chanel]: {
          ...channelsData[chanel],
      ...
      await instance.get(
        `/update?relay=${channelsData[chanel].chanel}&state=${state}`
      );
    } catch (err) {
      console.log("err", err);
  ...
  const renderHeader = () => {
    return (
      <View style={styles.header}>
        <Text
          style={StyleSheet.flatten([styles.title, { color: colors["#FFF"] }])}
        >
          Remote
        </Text>
      </View>
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.toolbarContainer}>
          ...
          <TouchableWithoutFeedback onPress={onToogleTheme}>
            ...
            >
              <MaterialIcons
                name={theme === "dark" ? "nightlight-round" : "wb-sunny"}
                size={getResponsiveDimention(62.09)}
                color={colors["#FFF"]}
              />
            </View>
      ...
      >
        {loading ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color={colors["#E1F488"]} />
          </View>
        ) : (
